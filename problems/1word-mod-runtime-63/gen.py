...
# ///
"""1 語の剰余 a % b (b は入力、2 <= b < 2^63) の入力を作る。seed を 1 つ受け取り、その番号のケースを stdout に出す。

期待出力は参照実装 (problem.toml の reference) がハーネスと一緒に作るので、ここでは入力だけを書く。
旧 judge の 1word-mod/runtime/1word-63/gen/make_inputs.py の手書きケースと乱数ケースを、その順番のまま seed に割り当てた。
乱数の種も同じなので、中身は旧 judge の testcases/*.in と一致する。

入力: N b sa ma mb (2 <= b < 2^63。a_i は LCG で n 個作る)
"""
import random
import sys
...
def cases() -> list[tuple[str, str]]:
    out = [
        ...
        ("edge_b_pow2", case(1_000_000, 1 << 40, 1, LCG_MA, LCG_MB)),
    ]
