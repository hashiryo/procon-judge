#pragma once
// v50 に、v55 で入れた x^(2^32-1) の作り方の変更だけを入れた版。表は v50 のまま。
//
// v50 は mul2 の片側で g = frob32(x)·frob16(N) を作り、表から引いた x^M の逆元に g と
// frob32(x) を順に掛けていた。mul2 に渡す frob32(x) を frob32(x)^2 に替えると、出力の
// w = frob32(x)^2·frob16(N) に frob32(x) を掛けるぶんが先に入るので、表を引いたあとの
// 掛け算は逆元に w を掛ける 1 回で済む。frob32(x)^2 の sq は表を引く前に、N や frob16(N)
// と並んで計算できる。掛け算が 1 本減り、sq が 1 本増える。
//
// v55_2 (log から逆元の識別子を引く版) とは掛け算の本数も表の大きさも同じで、違うのは
// 逆元の識別子を log と同じ 1 回の表引きで取れることだけ (表引きが 1 段少ない)。v55 と
// v55_2 の差を、表の組み方によるものとこの変更によるものに分けて見るための版。
//
...
namespace gf2_64_log_pohlig_v55_3 {
...
u64 log_g(u64 x) {
 ...
 // x^(2^32-1) = frob32(x) / x = w / x^M なので、x^M の逆元に w を 1 回掛けるだけで出る
 const u64 s= mul(embed_idx(u16(lnv >> 16)), w);
 u64 s7= frob7(s);
...
inline vector<u64> run(const vector<u64>& xs) {
 using gf2_64_log_pohlig_v55_3::log_g;
