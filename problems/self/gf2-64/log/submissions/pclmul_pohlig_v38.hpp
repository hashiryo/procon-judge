#pragma once
// v37_2 の baby step の表を、実行時構築から constexpr に替えた版。
//
// gf2-64-log はケースが 5 つでクエリが 1 万程度なので、記録の 4 割ほどがプロセスごとの
// 表作り (13 万回の乗算と 2 MB の書き潰し) だった。65537 の表と同じように constexpr で
// 焼いてしまえば、これが丸ごと消えて .rodata から読むだけになる。
//
// 焼くために build() の中身を書き換えてある。frob32 と乗算は byte table で回し、
// EMPTY を 0 にして (値の側に +1 して 0 を避ける) 表の初期化ループも省いた。
// 65537 の側は v35_2 / v37_2 と同じで、N = x^(2^32+1) の座標の比を添字にする。
// BSGS の探索そのもの (± 対称性, m = 65536) は v37_2 と同じ。
//
...
namespace gf2_64_log_pohlig_v38 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v38::log_g;
