#pragma once
// v55 (x^M ∈ F_2^16 の逆元を log から作る版) の、逆元を冪の表の積ではなく、log から
// 逆元の識別子を引く表と v50 の embed_idx で作るようにした版。
//
// v55 は G_65535^-a と G_65535^-256b の表 (合わせて 4 KB) を引いて掛けていた。ここでは
// 「log r → 逆元の識別子」の表 (65535 要素、128 KB) を持ち、引いた識別子を v50 と同じ
// embed_idx で 64 bit の元に戻す。逆元の log は 65535 - r だが、表には逆元の側を入れて
// あるので引き算は要らない。識別子どうしは掛け算を通さないと組み合わせられないので、
// この表は v55 のように 2 枚の小さい表に分けられない。F_2^16 の表は LN16 と合わせて
// v50 と同じ 256 KB に戻り、表引きも LN16、この表、embed_idx の 3 段が続く。
//
// 代わりに冪の表どうしを掛ける 1 本が要らない。v55 と同じく mul2 の片側で
// frob32(x)^2·frob16(N) を作っておくので、x^(2^32-1) は逆元に 1 回掛けるだけで出る。
// 掛け算は v55 より 1 本少なく、v50 (逆元に g と frob32(x) を順に掛ける 2 本) と比べても
// 1 本少ない。
//
...
namespace gf2_64_log_pohlig_v55_2 {
...
// =============================================================================
// EXPNEG16: F_2^16 の「log → その逆元の識別子」の表 (128 KB)。LN16 を逆向きに引く exp 表
// だが、逆元の log は 65535 - log なので、最初から逆元の側を入れて引き算を省いてある。
// =============================================================================
struct ExpNeg16 {
 u16 t[65535];
};
constexpr ExpNeg16 EXPNEG16= []() {
 ExpNeg16 r{};
 for(u32 id= 1; id < 65536; ++id) {
  const u32 l= LN16.t[id];
  r.t[l ? u32(P_F16) - l : 0]= u16(id);
 }
 return r;
}();
...
u64 log_g(u64 x) {
 ...
 // x^(2^32-1) = w / x^M。x^M の逆元の識別子を log から引き、64 bit の元に戻して掛ける
 const u64 s= mul(embed_idx(EXPNEG16.t[r1]), w);
 u64 s7= frob7(s);
...
inline vector<u64> run(const vector<u64>& xs) {
 using gf2_64_log_pohlig_v55_2::log_g;
