#pragma once
// v40 (表を constexpr + frob32 を 4 本まとめ) の表を、fingerprint と payload の 2 本に
// 分けた版。
//
// 探索中に触るのは fingerprint の側だけなので、そこを u32 にすると 2 MB の u64 1 本が
// 1 MB になり、1 本のキャッシュラインに 8 個から 16 個入るようになる。payload は
// 当たったときしか読まないので、そちらは冷たいままでよい。
//
// 照合が漏れないことは bit の勘定で押さえてある。線形探索は空きに当たるまで隣の slot へ
// ずれていくので、見つけた entry の本来の slot が今の h と同じとは限らない。つまり
// slot の bit は照合の役に立たず、fingerprint だけで鍵が決まる必要がある。鍵
// κ = z + z^-1 は frob32 で固定される 32 次元の部分空間を動くので、κ の bit 0..32 を
// 照合すれば一意に決まる (この 33 bit を落とした空間と部分空間の交わりが 0 であることを
// 確認済み。32 bit の連続窓で足りる取り方は無かった)。u32 には空きの印の 1 bit を含めて
// bit 0..30 しか入らないので、残りの bit 31..32 は payload 側に置いて、当たった候補に
// 対してだけ確かめる。
//
// 65537 側と探索の構造は v40 と同じ。
//
...
namespace gf2_64_log_pohlig_v41 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v41::log_g;
