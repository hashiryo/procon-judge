...
constexpr Ln16Inv LNINV16= []() {
 ...
 Ln16Inv r{};
 // log は ((2^64-1)/65535)^-1 = 16384 倍 (mod 65535) して持つ (2699·16384 ≡ 49826)
...
constexpr Ln641 LN641= []() {
 ...
 // log は ((2^64-1)/641)^-1 = 590 倍 (mod 641) して持つ
...
constexpr ClassTable65537 CLS65537= []() {
 ...
 for(u32 k= 0; k < 65537; ++k) {
  ...
  cur= MUL_G17(cur);
  // log (-2k) は ((2^64-1)/65537)^-1 = 16384 倍 (mod 65537) して持つので、1 歩で -32768
...
inline u64 ln(u64 x) {
 ...
 // 6700417 以外の表の値は r·((2^64-1)/m)^-1 mod m なので、(2^64-1)/m を掛けるだけで CRT の項になる (積は 2^64-1 未満)
