#pragma once
// v50 の、x^M ∈ F_2^16 の逆元を表から引くのをやめて、log から作るようにした版。
//
// v48 からは、F_2^16 の log と逆元の識別子を 1 つの u32 に詰めた表 (256 KB) を引いていた。
// 逆元の log は 65535 - log なので、log さえ引ければ逆元は G_65535 = 2^M の冪として作れる。
// 冪を 1 枚の表で引くと 128 KB の表がもう 1 枚要り、表引きも 2 段になる。そこで指数を
// 下位と上位の 8 bit に分け、G_65535^-a と G_65535^-256b の表 (合わせて 4 KB) を引いて
// 掛ける形にした。表には負の冪を入れておくので、65535 から引く計算も要らない。
// これで F_2^16 の表は log だけの u16 (128 KB) になり、比率方式が log を引く 2 回も
// 1 要素 2 byte に戻る。
//
// 冪の表の積で掛け算が 1 本増えるので、frob32(x)·frob16(N) を作っていた mul2 の片側を
// frob32(x)^2·frob16(N) に替えて、最後に frob32(x) を掛ける 1 本を消した。掛け算の本数は
// v50 と同じで、sq が 1 本増える。表を引いてから x^(2^32-1) が出るまでの掛け算は
// v50 と同じ 2 段で、v50 の識別子を 64 bit に戻す XOR が無くなる。
//
...
namespace gf2_64_log_pohlig_v55 {
...
constexpr u64 pow_ce(u64 a, u64 e) {
 u64 r= 1;
 for(; e; e>>= 1, a= mul_ce(a, a))
  if(e & 1) r= mul_ce(r, a);
 return r;
}
// =============================================================================
// LN16: F_2^16 の「識別子 → log」の表 (128 KB)。v50 の LNINV16 から逆元の半分を落としたもの。
...
constexpr Ln16Table LN16= []() {
 ...
 for(u32 k= 0; k < 65535; ++k) {
  r.t[T_lo[u8(cur)] ^ T_hi[cur >> 8]]= u16(k * 2699 % 65535);
  cur= u16(cur << 1) ^ (0x002DU & -u16(cur >> 15));
 }
 return r;
}();
constexpr u16 ln16(u16 idx) { return LN16.t[idx]; }  // (log * 2699) mod 65535
// =============================================================================
// 定数群
// =============================================================================
...
// ln16 はこの元を底にした log になっている (2699 はそのための係数)
constexpr u64 G_65535= pow_ce(2, EXP_F16);
static_assert(ln16(u16(G_65535)) == 1, "ln16 の底が 2^M になっていない");
// =============================================================================
...
// =============================================================================
// NEG16: x^M の逆元を log から作るための、G_65535 の負の冪の表 (4 KB)。
// x^M の log が r なら逆元は G_65535^-r。r を下位と上位の 8 bit に分けて
// lo[a] = G_65535^-a、hi[b] = G_65535^-256b を持ち、lo[u8(r)] · hi[r >> 8] で作る。
// =============================================================================
struct NegPow16 {
 u64 lo[256], hi[256];
};
constexpr NegPow16 NEG16= []() {
 const RawByteTable m1= make_mul_table(pow_ce(G_65535, P_F16 - 1));  // G_65535^-1 倍
 NegPow16 r{};
 r.lo[0]= 1;
 for(int a= 1; a < 256; ++a) r.lo[a]= apply_raw(m1, r.lo[a - 1]);
 const RawByteTable m256= make_mul_table(apply_raw(m1, r.lo[255]));  // G_65535^-256 倍
 r.hi[0]= 1;
 for(int b= 1; b < 256; ++b) r.hi[b]= apply_raw(m256, r.hi[b - 1]);
 return r;
}();
...
u64 log_g(u64 x) {
 ...
 // x_f16 = N·frob16(N) = x^M ∈ F_2^16^*、w = frob32(x)^2·frob16(N) = x^(2^32+M-1)
 auto [x_f16, w]= unpack(mul2(_mm256_set_epi64x(0, sq(x32), 0, N), _mm256_set1_epi64x(fN)));
 const u16 r1= ln16(u16(x_f16));
 // x^(2^32-1) = w / x^M。x^M = G_65535^r1 なので、割る代わりに G_65535^-r1 を掛ける
 const u64 s= mul(mul(NEG16.lo[u8(r1)], NEG16.hi[r1 >> 8]), w);
 u64 s7= frob7(s);
...
inline vector<u64> run(const vector<u64>& xs) {
 using gf2_64_log_pohlig_v55::log_g;
