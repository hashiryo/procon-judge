#pragma once
// v50 の先読みを 12 個から 8 個に縮めた版 (prefetch は残す)。
//
// v50 は 3 段 (今の 4 本、次の 4 本、その次の 4 本) を持っているが、当たりが早いときは
// 3 段目が丸ごと無駄になる。2 段に減らすと無駄な乗算と生きている値が減る代わりに、
// prefetch を出してから probe するまでの距離が 12 歩から 8 歩に縮まる。
//
...
struct BSGSTable6700417 {
 ...
 static inline u32 solve(u64 target) {
  ...
  for(u8 i= 0; i <= max_i; i+= 4) {
   ...
   A= An, B= Bn;
   for(int j= 0; j < 4; ++j) t[j]= t_n[j];
   if(i + 8 <= max_i) {
    An= mul2(A, V_S4), Bn= mul2(B, V_S4);
    tie(t_n[0], t_n[1])= unpack(An), tie(t_n[2], t_n[3])= unpack(Bn);
    for(int j= 0; j < 4; ++j) _mm_prefetch((const char*)&TAB.t[u32(t_n[j]) & mask], _MM_HINT_T0);
   }
  }
  return q;
 }
