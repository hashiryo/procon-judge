...
# ///
"""GF(2^64) の除算 の入力を作る。seed を 1 つ受け取り、その番号のケースを stdout に出す。

期待出力は参照実装 (problem.toml の reference) がハーネスと一緒に作るので、
ここでは入力だけを書く。旧 judge の gf2-64-div/testcases/gen.py の種類を seed に割り当てた。
"""
import random
import sys
...
# seed -> (種類, 件数)
CASES = {0: ("sample", 8), 1: ("small", 100), 2: ("edge_one", 200), 3: ("structured", 1000), 4: ("random", 10000), 5: ("random", 100000)}
...
def make(kind: str, t: int, rng: random.Random) -> list:
    ...
    if kind == "sample":
        return [(0, 1), (1, 1), (2, 2), (3, 5), (5, 3), (MASK64, 1), (MASK64, MASK64),
    ...
    if kind == "edge_one":
        return [(rng.choice([0, 1, MASK64, rng.randint(0, MASK64)]), rng.choice([1, MASK64, nz()]))
                for _ in range(t)]
    if kind == "structured":
        return [((1 << rng.randint(0, 63)) | rng.randint(0, 7), (1 << rng.randint(0, 63)) | rng.randint(0, 7) | 1)
                for _ in range(t)]
