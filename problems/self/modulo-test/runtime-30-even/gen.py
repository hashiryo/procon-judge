...
# ///
"""剰余の反復 (mod は入力、2^20 < mod < 2^30 の偶数) の入力を作る。seed を 1 つ受け取り、その番号のケースを stdout に出す。

期待出力は参照実装 (problem.toml の reference) がハーネスと一緒に作るので、ここでは入力だけを書く。
旧 judge の modulo-test/runtime/runtime-30-even/gen/make_inputs.py の手書きケースと乱数ケースを、その順番のまま seed に割り当てた。
乱数の種も同じなので、中身は旧 judge の testcases/*.in と一致する。

入力: N mod s a b c d ((1 << 20) + 2 < mod < (1 << 30) - 2 の偶数)
"""
import random
import sys
...
def cases() -> list[tuple[str, str]]:
    out = [
        ...
        ("edge_01", case(20, MAX_MOD, MAX_MOD - 1, 1, MAX_MOD - 2, 1, MAX_MOD - 2)),
        ("edge_02", case(100, MAX_MOD, 123456789, 0, MAX_MOD - 1, 0, MAX_MOD - 1)),
        ("edge_pow2", case(1000, 1 << 29, 12345, 67890, 13579, 24680, 11111)),
    ]
