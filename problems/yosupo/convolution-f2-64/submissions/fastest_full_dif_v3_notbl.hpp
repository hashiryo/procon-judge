...
inline const __m256i RED256_DIF3= GF2_64_M256_SETR_EPI8(0, 27, 45, 54, 90, 65, 119, 108, 0, 0, 0, 0, 0, 0, 0, 0, 0, 27, 45, 54, 90, 65, 119, 108, 0, 0, 0, 0, 0, 0, 0, 0);
GNU_TARGET("vpclmulqdq") inline __m256i clmul_reduce_pair(__m256i a_vec, __m256i b_vec) {
 ...
 __m256i red_vec= _mm256_shuffle_epi8(RED256_DIF3, indices);
...
GNU_TARGET("vpclmulqdq") inline void nim_fft(std::vector<u64>& f) {
 ...
 for(int k= d; k >= 1; --k) {
  ...
  // j=0: ska=0 で butterfly_0 (GNU_TARGET("pclmul") 不要)
  {
   u64* base= f.data();
