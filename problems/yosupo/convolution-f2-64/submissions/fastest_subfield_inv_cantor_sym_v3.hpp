...
//
//   half=1 (len=2):                 i=0 のみ (GNU_TARGET("pclmul") なし)
//   half=2 (len=4):                 i=0 + i=1 scalar
//   half=N≥4:                       i=0 + i=1 scalar + pair(2,3)..pair(N-2,N-1)
//
// 利点として期待:
//   - i=0 (XOR-only) と i=1 (scalar GNU_TARGET("pclmul"), lat ~5) が dispatch 直後に発行され、
//     後続 pair ループとは独立に走るので OoO で latency を埋めやすい
//   - ブロック末尾で VPCLMULQDQ (mul2) で締めることで、 次ブロックの prefetch /
//     L1 fetch とパイプが整列しやすい (tail 版だと末尾 scalar GNU_TARGET("pclmul") のあと次の
...
GNU_TARGET("vpclmulqdq") inline void nim_fft(std::vector<u64>& f) {
 ...
 while(len < n) {
  ...
  for(int l= 0; l < n; l+= len) {
   ...
   // i = 0: g0[0] = 0 → GNU_TARGET("pclmul") なし
   {
    u64 lo= f0[0], hi= f1[0];
    f0[0]= lo;
    f1[0]= lo ^ hi;
