...
// 多項式乗算だけ GNU_TARGET("pclmul") を使わずに**ビット単位 XOR シフトのループ**で実装する。
// 単純な GF(2) 多項式乗算アルゴリズムの教科書版。
//
// 同じ algorithm 構造で「GNU_TARGET("pclmul") の効果がどれくらいあるか」を比較するための
...
template <const auto& COLS> constexpr auto make_byte_tables() {
 ...
 for(int pos= 0; pos < 8; ++pos)
  for(int col= 0; col < 8; ++col)
   for(int mask= 0; mask < (1 << col); ++mask) T[pos][mask | (1 << col)]= T[pos][mask] ^ COLS[pos * 8 + col];
 return T;
}
