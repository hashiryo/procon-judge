...
# ///
"""GF(2^64) の積 の入力を作る。seed を 1 つ受け取り、その番号のケースを stdout に出す。

期待出力は参照実装 (problem.toml の reference) がハーネスと一緒に作るので、
ここでは入力だけを書く。旧 judge の gf2-64-mul/testcases/gen.py の種類を seed に割り当てた。
"""
import random
import sys
...
def make(kind: str, t: int, rng: random.Random) -> list:
    ...
    if kind == "edge_zero_one":
        return [(rng.choice([0, 1, MASK64, rng.randint(0, MASK64)]),
                 rng.choice([0, 1, MASK64, rng.randint(0, MASK64)])) for _ in range(t)]
    if kind == "structured":
