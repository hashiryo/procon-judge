#pragma once
// v47 (x^(2^32-1) を逆元経由で作る版) の、F_2^16 の log 表と逆元表を 1 枚にまとめた版。
//
// v47 は同じ添字 u16(x^M) で log の表 (128 KB) と逆元の表 (128 KB) を 2 回引いていた。
// 値がどちらも 16 bit なので 1 つの u32 に詰められて、x^M のところは 1 回の表引きで
// 両方取れる。代わりに、比率方式が log だけのために引く 2 回は 1 要素が 2 byte から
// 4 byte になり、キャッシュラインあたりの要素数が半分になる。表の総量は 256 KB で
// v47 と変わらない。どちらが得かを測るための版。
//
...
namespace gf2_64_log_pohlig_v48 {
...
constexpr Ln16Inv LNINV16= []() {
 ...
 Ln16Inv r{};
 for(u32 k= 0; k < 65535; ++k) r.t[id[k]]= ((u32(k) * 2699 % 65535) << 16) | id[k ? 65535 - k : 0];
 return r;
}();
constexpr u16 ln16(u16 idx) { return u16(LNINV16.t[idx] >> 16); }   // (log * 2699) mod 65535
constexpr u16 inv16(u16 idx) { return u16(LNINV16.t[idx]); }        // 逆元の識別子
// =============================================================================
// 定数群
// =============================================================================
...
constexpr ClassTable65537 CLS65537= []() {
 ...
 for(u32 k= 0; k < P_F17; ++k) {
  ...
  if(b1 == 0) {         // 類 ∞ = F_2^16 自身。u = 1 なので log は 0
   ...
   u32 idx= (LNINV16.t[b0] >> 16) + u32(P_F16) - (LNINV16.t[b1] >> 16);
...
u64 log_g(u64 x) {
 ...
 const u64 s= mul(mul(embed_idx(u16(lnv)), g), x32);
 u64 s7= frob7(s);
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v48::log_g;
