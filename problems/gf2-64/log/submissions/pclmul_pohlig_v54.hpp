#pragma once
// v52 (先読みを 1 段だけにした版) に prefetch を戻した版。
//
// 先読みの深さと prefetch の有無を分けて測るための 4 つ目。v50 が 3 段 + prefetch、
// v51 が 3 段のみ、v52 が 1 段のみ、これが 1 段 + prefetch にあたる。
// アドレスが分かってから叩くまでの距離は 4 歩ぶんしかないが、その間に次の 4 本の
// 掛け算が挟まるので、多少の余裕にはなる。
//
...
namespace gf2_64_log_pohlig_v54 {
...
struct BSGSTable6700417 {
 ...
 GNU_TARGET("pclmul,vpclmulqdq") static inline u32 solve(u64 target) {
  ...
  for(u8 i= 0; i <= max_i; i+= 4) {
   ...
   for(int j= 0; j < 4; ++j) _mm_prefetch((const char*)&TAB.t[u32(t[j]) & mask], _MM_HINT_T0);
   const __m256i An= mul2(A, V_S4), Bn= mul2(B, V_S4);  // 次の 4 本を先に出しておく
   for(int j= 0; j < 4; ++j) {
    if(i + j > max_i) break;
    u64 tt= t[j];
    u32 h= u32(tt) & mask;
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v54::log_g;
