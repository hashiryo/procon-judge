#pragma once
// v41 の solve から、定数ベクタの組み立てと unpack / 詰め直しを取り除いた版。
//
// 2 つ直した。1 つ目は定数で、_mm256_set_epi64x(0, step_2, 0, step_1) のようなものを
// 関数の中で組むと、手元の clang は毎回 movk の連鎖で即値を作って vector に差し込む
// (solve 1 回につき movk 41 本 + 差し込み 30 本ほど出ていた)。関数の外の static に
// しておけばロード数本で済む。
//
// 2 つ目は詰め直しで、mul2 の結果は q0 と q2 に乗る。これは次の mul2 の operand の
// 置き場所そのものなので、unpack してスカラーに落としてから set_epi64x で組み直すのは
// 無駄になる。stream を (0,1) と (2,3) の 2 本のベクタで持ち、次の段へはベクタのまま
// 渡す形にした。スカラーは frob32 と probe のために取り出すが、組み直しは無くなる。
// そのぶん先頭の 1 レーンが target × 1 の空打ちになるが、クエリに 1 回だけ。
//
// 表と 65537 側は v41 と同じ。
//
...
namespace gf2_64_log_pohlig_v42 {
...
// =============================================================================
// κ の fingerprint。bit 0..30 に、空きと区別する最上位 bit を足したもの。
constexpr u32 fp_of(u64 k) { return 0x80000000u | u32(k & 0x7FFFFFFFu); }
// payload の上位に置く、fingerprint に入り切らなかった bit 31..32
constexpr u32 hi_of(u64 k) { return u32((k >> 31) & 3); }
// frob32 を 4 本まとめて引く。表の XOR を packed のまま畳む。
inline __m256i frob32_4lane(u64 a0, u64 a1, u64 a2, u64 a3) {
 using gf2_64_pclmul::FROB32_BYTE;
 __m256i v= _mm256_set_epi64x(FROB32_BYTE[0][u8(a3)], FROB32_BYTE[0][u8(a2)], FROB32_BYTE[0][u8(a1)], FROB32_BYTE[0][u8(a0)]);
 for(int i= 1; i < 8; ++i) v= _mm256_xor_si256(v, _mm256_set_epi64x(FROB32_BYTE[i][u8(a3 >> (8 * i))], FROB32_BYTE[i][u8(a2 >> (8 * i))], FROB32_BYTE[i][u8(a1 >> (8 * i))], FROB32_BYTE[i][u8(a0 >> (8 * i))]));
 return v;
}
// k[j] = a[j] + frob32(a[j])。XOR は GPR 側で取る (ymm に詰め直すより安い)。
inline void keys4(const u64 (&a)[4], u64 (&k)[4]) {
 const __m256i f= frob32_4lane(a[0], a[1], a[2], a[3]);
 k[0]= a[0] ^ u64(_mm256_extract_epi64(f, 0));
 k[1]= a[1] ^ u64(_mm256_extract_epi64(f, 1));
 k[2]= a[2] ^ u64(_mm256_extract_epi64(f, 2));
 k[3]= a[3] ^ u64(_mm256_extract_epi64(f, 3));
}
struct BSGSTable6700417 {
 ...
 struct Tab {
  u32 fp[mask + 1];   // 探索で触るのはこちらだけ
  u32 pay[mask + 1];  // 当たったときだけ読む (bit 18..19 に κ の bit 31..32)
 };
 static constexpr Tab TAB= []() {
  ...
  for(u32 j= 0; j <= m; ++j) {
   ...
   while(r.fp[h]) h= (h + 1) & mask;
   r.fp[h]= fp_of(key);
   r.pay[h]= (hi_of(key) << 18) | ((u32(j) + 1) << 1) | u32(cur > fr);
   cur= apply_raw(mg, cur);
  }
  return r;
 }();
 // 定数のベクタは 1 度だけ作る (関数の中で組むと毎回即値の組み立てが走る)
 static inline const __m256i V_S01= GF2_64_M256_SET_EPI64X(0, step_1, 0, 1);
 static inline const __m256i V_S23= GF2_64_M256_SET_EPI64X(0, step_3, 0, step_2);
 static inline const __m256i V_S4= GF2_64_M256_SET1_EPI64X(step_4);
 u32 solve(u64 target) const {
  // stream は (0,1) と (2,3) の 2 本のベクタで持つ。mul2 の出力 (q0, q2) がそのまま
  // 次の mul2 の operand の置き場所なので、段を進めるのに詰め直しが要らない。
  const __m256i tv= _mm256_set1_epi64x(target);
  __m256i A= mul2(tv, V_S01), B= mul2(tv, V_S23);       // (t0, t1), (t2, t3)
  __m256i An= mul2(A, V_S4), Bn= mul2(B, V_S4);         // (t4, t5), (t6, t7)
  __m256i An2= mul2(An, V_S4), Bn2= mul2(Bn, V_S4);     // (t8, t9), (t10, t11)
  ...
  keys4(t, k), keys4(t_n, k_n), keys4(t_n2, k_n2);
  for(int j= 0; j < 4; ++j) {
   _mm_prefetch((const char *)&TAB.fp[u32(k[j] >> 1) & mask], _MM_HINT_T0);
   _mm_prefetch((const char *)&TAB.fp[u32(k_n[j] >> 1) & mask], _MM_HINT_T0);
   _mm_prefetch((const char *)&TAB.fp[u32(k_n2[j] >> 1) & mask], _MM_HINT_T0);
  }
  for(u8 i= 0; i <= max_i; i+= 4) {
   for(int j= 0; j < 4; ++j) {
    if(i + j > max_i) break;
    const u64 tt= t[j], kk= k[j];
    const u32 want= fp_of(kk), hi= hi_of(kk);
    u32 h= u32(kk >> 1) & mask;
    while(TAB.fp[h]) {
     if(TAB.fp[h] == want && (TAB.pay[h] >> 18) == hi) {
      const u32 val= TAB.pay[h], jj= ((val & 0x3FFFFu) >> 1) - 1;
      u64 n= u64(i + j) * gstep;
      if(n >= q) n-= q;
      n+= (u64(tt > (tt ^ kk)) == (val & 1)) ? jj : q - jj;
   ...
   if(i + 12 <= max_i) {
    ...
    tie(t_n2[0], t_n2[1])= unpack(An2), tie(t_n2[2], t_n2[3])= unpack(Bn2);
    keys4(t_n2, k_n2);
    for(int j= 0; j < 4; ++j) _mm_prefetch((const char *)&TAB.fp[u32(k_n2[j] >> 1) & mask], _MM_HINT_T0);
   }
  }
  return q;
 }
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v42::log_g;
