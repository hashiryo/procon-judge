#pragma once
// v38 (baby step の表を constexpr にした版) の giant step から frob32 を追い出した版。
//
// 探索中の値は z_i = target · g^(-2mi) なので、その逆元も z_i^-1 = target^-1 · g^(+2mi)
// とやはり等比数列になる。そこで z と z^-1 を 2 本並べて進めれば、鍵 κ = z + z^-1 は
// XOR 1 回で済み、1 歩ごとの frob32 (8 引き + 7 XOR、しかも結果を GPR へ戻す) が消える。
// 代わりに乗算が 1 歩 1 本増えるが、z 側と z^-1 側は 1 本の mul2 に相乗りできる。
// target^-1 = frob32(target) だけはクエリの頭で 1 回計算する。
//
// ± の判定も z と z^-1 の大小そのままになる。表と 65537 側は v38 と同じ。
//
...
namespace gf2_64_log_pohlig_v39 {
...
struct BSGSTable6700417 {
 ...
 u32 solve(u64 target) const {
  ...
  // z[j] = target · g^(-2m j), w[j] = z[j]^-1。1 本の mul2 で両側を同時に進められる。
  z[0]= target, w[0]= iv;
  const __m256i zw= _mm256_set_epi64x(0, iv, 0, target);
  tie(z[1], w[1])= unpack(mul2(zw, _mm256_set_epi64x(0, up_1, 0, step_1)));
  tie(z[2], w[2])= unpack(mul2(zw, _mm256_set_epi64x(0, up_2, 0, step_2)));
  tie(z[3], w[3])= unpack(mul2(zw, _mm256_set_epi64x(0, up_3, 0, step_3)));
  const __m256i s4v= _mm256_set_epi64x(0, up_4, 0, step_4);
  for(int j= 0; j < 4; ++j) tie(zn[j], wn[j])= unpack(mul2(_mm256_set_epi64x(0, w[j], 0, z[j]), s4v));
  for(int j= 0; j < 4; ++j) {
  ...
  for(u8 i= 0; i <= max_i; i+= 4) {
   ...
   for(int j= 0; j < 4; ++j) z[j]= zn[j], w[j]= wn[j];
   if(i + 8 <= max_i)
    for(int j= 0; j < 4; ++j) {
     tie(zn[j], wn[j])= unpack(mul2(_mm256_set_epi64x(0, w[j], 0, z[j]), s4v));
     _mm_prefetch((const char *)&TAB.t[u32((zn[j] ^ wn[j]) >> 1) & mask], _MM_HINT_T0);
    }
  }
  return q;
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v39::log_g;
