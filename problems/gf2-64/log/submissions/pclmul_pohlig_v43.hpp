#pragma once
// v35 (65537 を constexpr の直接引き表にした版) の baby step の表も constexpr にした版。
// BSGS は ± 対称性を入れる前の形 (v34_3 と同じ、m = 131072、探索 53 回) のまま。
//
// x64 の記録を見ると、対称性で探索を半分にする得より、鍵が z + frob32(z) になって
// prefetch のアドレスが frob32 の後ろにぶら下がる損の方が大きかった (v35 が v36 と v37 の
// どちらにも勝っている)。なので対称性は入れずに、表作りだけを compile 時へ移す。
//
// 鍵が元そのものなら baby step の構築に frob32 が要らないので、1 反復あたりの constexpr の
// 仕事が半分で済み、13 万反復でも clang の step 上限に収まる (対称性ありの m = 131072 は
// 収まらなかった)。空きの印は 0 にして、値の側を j + 1 にしてある。
//
...
namespace gf2_64_log_pohlig_v43 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v43::log_g;
