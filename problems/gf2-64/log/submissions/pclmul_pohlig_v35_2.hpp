#pragma once
// v35 の 65537 の添字の作り方を、比を使う方に替えた版。
//
// v35 は x^(2^32-1) を作って y = x + x^-1 の 16 bit を添字にしていた。こちらは
// N = x^(2^32+1) の F_2^16 座標の比を添字にする。N は関数の 1 行目で、frob16(N) も
// x_f16 のために既に計算してあるので、x^(2^32-1) を作る mul と frob32、y を作る
// frob16 が消える。代わりに F_2^16 の log 表を 2 回余計に引く。詳しくは下の
// ClassTable65537 のコメント。
//
// 641 の完全ハッシュと 6700417 の BSGS は v34_3 のまま。
//
...
namespace gf2_64_log_pohlig_v35_2 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v35_2::init_tables;
  using gf2_64_log_pohlig_v35_2::log_g;
  init_tables();
