#pragma once
// v38 (baby step の表を constexpr にした版) の frob32 を 4 本まとめて引く版。
//
// frob32 は byte table 8 本の XOR なので、4 本ぶんを packed の ymm のまま畳める
// (pow の byte_window 系が frobK_2lane でやっているのと同じ形)。スカラーだと
// 4 × 7 = 28 回の XOR が 7 回の vpxor になる代わりに、表から引いた値を ymm に詰める
// vpinsrq と、鍵をハッシュの添字にするために GPR へ戻す vpextrq が増える。
// BSGS は pow の窓と違って結果をそのまま mul2 に流せないので、この出し入れが
// 見合うかどうかは測ってみないと分からない。
//
// 表と 65537 側と探索の構造は v38 と同じ。
//
...
namespace gf2_64_log_pohlig_v40 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v40::log_g;
