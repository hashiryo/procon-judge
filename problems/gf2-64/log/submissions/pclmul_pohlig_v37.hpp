#pragma once
// v36 (6700417 の BSGS に ± 対称性を入れた版) の baby step を半分にした版。
//
// 対称性で 1 回の giant step が倍の幅を覆うので、baby step を m = 65536 に半分にしても
// 探索の回数は v34_3 と同じ 52 回に収まる。表は 4 MB から 2 MB になり、作る時間も半分に
// なる。gf2-64-log はケース 5 つでクエリが 1 万程度、しかも 5 ケース中 4 ケースが 1000
// クエリ以下なので、1 プロセスあたりの表作りが記録のかなりの部分を占める。どちらが
// 得かは問題のクエリ数しだいなので、m を変えただけの版を並べてある。
//
// 65537 側は v35 と同じ:
//
...
namespace gf2_64_log_pohlig_v37 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v37::init_tables;
  using gf2_64_log_pohlig_v37::log_g;
  init_tables();
