#pragma once
// v50 から prefetch と先読みの両方を外した版。
//
// 残したのは「次の 4 本の掛け算を先に出してから今の 4 本を probe する」ことだけで、
// 12 個先まで作り置きするのをやめた。当たりが早いクエリで無駄になる乗算が減り、
// 生きている値が 12 個から 8 個に減るのでレジスタも楽になる。代わりに、probe の
// アドレスが分かってから実際に叩くまでの余裕が無くなる。
//
...
namespace gf2_64_log_pohlig_v52 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v52::log_g;
