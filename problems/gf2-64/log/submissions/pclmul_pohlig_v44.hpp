#pragma once
// v40 (表 constexpr + frob32 を 4 本まとめ) に、v42 の変更だけを載せた版。
//
// v42 は v41 の「表を fingerprint と payload に分ける」変更の上に乗っていたが、
// x64 の記録ではその分割は損 (v40 5.27 ms → v41 5.92 ms, gcc) で、v42 の変更自体は
// 得 (v41 5.92 → v42 5.76) だった。なので分割を戻して v42 の変更だけを残す。
//
// v42 の変更は 2 つ。定数のベクタを関数の外の static にすること (関数の中で組むと
// 手元の clang は毎回 movk の連鎖で作っていた) と、mul2 の出力 (q0, q2) が次の mul2 の
// operand の置き場所そのものなので、unpack してスカラーに落としてから組み直すのを
// やめて、stream を 2 本のベクタのまま回すことの 2 つ。
//
...
namespace gf2_64_log_pohlig_v44 {
...
struct BSGSTable6700417 {
 ...
 u32 solve(u64 target) const {
  ...
  for(int j= 0; j < 4; ++j) {
   _mm_prefetch((const char *)&TAB.t[u32(k[j] >> 1) & mask], _MM_HINT_T0);
   _mm_prefetch((const char *)&TAB.t[u32(k_n[j] >> 1) & mask], _MM_HINT_T0);
   _mm_prefetch((const char *)&TAB.t[u32(k_n2[j] >> 1) & mask], _MM_HINT_T0);
  }
  for(u8 i= 0; i <= max_i; i+= 4) {
   ...
   if(i + 12 <= max_i) {
    ...
    tie(t_n2[0], t_n2[1])= unpack(An2), tie(t_n2[2], t_n2[3])= unpack(Bn2);
    keys4(t_n2, k_n2);
    for(int j= 0; j < 4; ++j) _mm_prefetch((const char *)&TAB.t[u32(k_n2[j] >> 1) & mask], _MM_HINT_T0);
   }
  }
  return q;
 }
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v44::log_g;
