#pragma once
// v50 の prefetch を外した版。
//
// v50 は 12 個先まで計算して prefetch しているが、当たるのは平均で 26 歩目あたりなので、
// 先読みのうち後ろの方はしばしば無駄になる。表は 4 MB で L3 には載るから、prefetch が
// 無くても probe は L3 のレイテンシで済み、4 本の独立な probe で重ねられるなら元が
// 取れていない可能性がある。それを見るために prefetch だけを外した (先読みの深さと
// 乗算の並びは v50 のまま)。
//
...
namespace gf2_64_log_pohlig_v51 {
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& xs) {
  using gf2_64_log_pohlig_v51::log_g;
