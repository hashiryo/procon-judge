...
// 計測区間の時間を stderr に 1 行の JSON で出す。実行側はこの接頭辞の行だけを拾う。
// メモリはハーネスでは測らない。Linux では pj が差し込む rss_preload が終了時の VmHWM を
// 同じ形の行で報告し、macOS では pj が ru_maxrss を読む (pj/execute.py の peak_rss_kb)。
