...
インターフェースは問題ごとに決めますが、形は 2 つに寄せます。クエリを順に処理する問題は、構築に初期状態だけを渡して計測区間の中でクエリを 1 つずつ渡す `Solver` にします。先の入力を構築へ渡すと、オフラインの前処理を計測区間の外に置ける実装だけが有利になるからです。1 回の計算で答えが出る問題は、構築と `run()` と `answer()` の 3 段に分けます。内部表現への変換が計測区間の外に出ます。関数 1 本で足りる問題は、構造体の静的メンバ関数にせず、自由関数 `run` を実装させます (`vector<u64> run(const vector<u64>& a)` のような形)。
