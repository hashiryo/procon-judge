...
`id` は置き場所から決まる名前と一致させます。`problems/` の下は何段でも掘れて、そこからの各段を `-` で繋いだものが id です。`problems/atcoder/abc172-d/` なら `atcoder-abc172-d`、`problems/self/gf2-64/pow/` なら `self-gf2-64-pow` です。1 段で置けばディレクトリ名がそのまま id です。検証で一致しなければエラーにし、同じ id が 2 か所から出てもエラーにします。判定サイトの問題は接頭辞のディレクトリ (`atcoder/`、`aoj/`、`yosupo/`、`yuki/`、`loj/` など) に置いてあります。自作の問題は `self/` の下で族ごとに分け (`self/gf2-64/`、`self/modulo-test/` など)、族を持たないものは `self/` の直下に置きます。2026-09-23 に平らな置き方から階層に移し (「問題ディレクトリの階層化とキーの -I の正規化の記録」)、2026-09-25 に自作の問題を `self/` の下に集めました (「自作の問題を self/ に集めた記録」)。
...
インターフェースは問題ごとに決めます。共通の signature を 1 つ決めるのは無理なので、各問題の `base.cpp` が期待する型と関数を宣言します。形は自由で、構造体と自由関数のどちらでもかまいません。クエリを順に処理する問題なら、状態のある型が自然です。1 回の計算で答えが出る問題なら関数 1 本で足ります。関数 1 本で足りるときは、構造体の静的メンバ関数にせず、自由関数 `run` にします。静的メンバ関数を包む構造体は名前空間の代わりにしかならないからです (「静的メンバ関数の入口を自由関数にした記録」)。

```cpp
// 1 回の計算で答えが出る問題の例。
// 提出側に vector<mint> run(const vector<mint>&, const vector<mint>&) を実装させる。
  auto t0 = chrono::steady_clock::now();
  auto c = run(a, b);
  auto t1 = chrono::steady_clock::now();
```
...
メモリは `base.cpp` の中で測ろうとしないでください。子が終わるときの報告と、実行側の `wait4` で拾います。

`wait4(pid, &status, 0, &ru)` は子が異常終了したあとでも `ru_maxrss` を返すので、OOM による強制終了や segfault のあとでも値が残ります。ただし Linux では上のとおり `pj` 自身のピークが乗るので、子の値として使えるのはそれを上回ったときだけです。
...
自由関数 1 本で返すより段が 1 つ多いですが、内部表現が入出力と違う問題では変換が計測区間の外に出ます。O(N) の変換を O(N log N) の計算と一緒に測ると、実装がどの表現を選んだかで乗り方が変わります。
...
`kind = "base"` の問題では、参照実装も提出と同じ形にしてください。`reference` に `.hpp` を指定します。ハーネスと一緒にコンパイルして期待出力を作ります。こうすると入力の解析を二重に書かなくて済みます。`kind = "raw"` の問題なら、参照実装は単体で動く `.cpp` になります。参照実装を `submissions/reference.hpp` に置いて `reference` にそのパスを書けば、期待出力を作るものがそのまま順位表の 1 行になります (`self-gf2-64-*` はこの形)。
...
pj try FILE [--input F | --problem ID --case NAME] [--env NAME]
                                             1 ファイルを提出と同じ探索パスで組み、自分の入力か問題のケースで走らせる (printf デバッグ用)
pj try --problem ID --submission PATH [--input F | --case NAME]
                                             提出をハーネスごと組み、自分の入力か問題のケースで走らせる。判定も記録もしない
...
実行した結果はそのまま追記します。raw の問題では記録が作られるのは必ず新しいキーのときです。base の問題は束ごとに同じキーの記録が 1 件ずつ増えます。取り込みの重複排除は (問題, キー, 束) で見ます。キーには問題の id が入らないので、中身の同じ問題が 2 つあると同じキーになります。2026-09-25 までは (キー, 束) で見ていました。そのため、二重に入っていた AtCoder の 1 問 (atcoder-code-festival-2014-exhibition-b) の記録が、run のたびに片方だけ捨てられていました。その問題は、url のある atcoder-code-festival-exhibition-b を残して消しました。
...
1 段目から 3 段目でソースに足した宣言は残しました。土台と重なる宣言は、組んだ中身も展開のされ方も変えません。判定サイトへ出すコードにとっては、直した意味が残ります。gf2-64 の家族の領域は、あとで土台に無い命令だけにしました (下の「gf2-64 の家族の領域は土台に無い命令だけにしました」)。

諦めたのは、土台の命令の宣言を書き忘れたコードを procon-judge で見つけることです。判定サイトへ出すときは、algo-workspace のバンドラが同じ一覧の GCC の pragma を提出の先頭に入れて補います。一覧は x86-64-v3 の命令 (xsave を除く) に pclmul と vpclmulqdq を足したものです。
...
同じ日に、Codeforces の G++20 と G++23 のカスタムテストでも、この形で組めて動くことを確かめました。vpclmulqdq の intrinsics を含むコードは実行時の分岐で飛ばされ、落ちませんでした。32 byte に揃えた配列をスタックに置く関数を、スタックを 16 byte ずつずらして呼んでも落ちず、Windows の GCC の揃え方の不具合には当たりませんでした。

GCC 13 以前の C++ では、pragma を書いても、そのあとの `__AVX2__` などの機能のマクロは立ちません。C では立ち、GCC 14 からは C++ でも立ちます。命令は使えるので CE にはなりませんが、マクロで経路を分けるコードは、AOJ (GCC 11) と Codeforces の G++20 (GCC 13) で遅い側に落ちます。経路を分けるときは `__x86_64__` で見ます。clang の push のときと同じ決まりです。
...
### gf2-64 の家族の領域は土台に無い命令だけにしました

土台をオプションで渡すようになったので、`_common.hpp` が開く領域の一覧 (x86-64-v3 の命令と pclmul) は、土台と重なるだけになりました。一覧を消し、提出が `GF2_64_EXTRA_TARGETS` を定義したときだけ、その命令で領域を開くようにしました。vpclmulqdq も土台に入ったので、86 本の `#define GF2_64_EXTRA_TARGETS "vpclmulqdq"` を消しました。残る定義は、gf2-64-sq の gfni.hpp の gfni だけです。base.cpp の最後の `GF2_64_TARGET_END` は、領域を開かなかったときは何もしません。

家族の 193 本を、消す前と後で x86 の GCC 15 と clang で組み、逆アセンブルが同じことを確かめました。消した定義はコマンドラインで渡し、assert の行番号をそろえて比べています。arm (SIMDe) では、前処理した結果が同じでした。

キーのフラグは変わりませんが、共通ヘッダと 86 本の中身が変わるので、家族の記録は 4 環境とも測り直しになりました。

測り直し (763967c、run 36109809081) では、家族の判定は変わりませんでした。同じ CPU モデルで比べられた x64 の 84 組の時間の比は、中央値 1.00 です。
...
切り替えた回の測り直し (763967c、run 36109809081) で、arm の判定は 1 件も変わりませんでした。測った時間の合計は、arm-gcc の家族が 609 秒から 450 秒 (0.74 倍) になり、pclmul を多く使う提出は 0.34 倍以下まで縮みました。家族の外と arm-clang は、どちらも 1.00 倍前後です。raw の小さな問題には 1 ケースだけ跳ねた記録がありますが、run の始めに arm のジョブが 40 本同時に立った時間帯に集まっていて、揺れと見ています。

arm-clang の 32 bit の Montgomery は、5 件 (提出では montgomery.hpp、lib-mo32.hpp、montgomery_even.hpp) が 2.1 倍から 2.25 倍遅くなりました。SVE があると 64 bit の掛け算をベクトルで組めるので、clang が鎖の中の独立な 2 つの掛け算を 1 つのベクトル演算にまとめ (SLP のベクトル化)、スカラーとの行き来が鎖に入ったためです。`+nosve` にするか、`-march=armv8.2-a+aes -mtune=neoverse-n2` にすると、元のスカラーの命令に戻ることを手元の clang で確かめました。arm は組めて正しく動けば足りるので、フラグは今のままにしています。

## 自作の問題を self/ に集めた記録

2026-09-25 に、自作の問題 51 問 (13 ディレクトリ) を `problems/self/` の下に移し、id の先頭に `self-` を付けました。`problems/gf2-64/pow/` は `problems/self/gf2-64/pow/` になり、id は `gf2-64-pow` から `self-gf2-64-pow` になります。サイトの問題一覧は id の順なので、`1word-mod` や `constexpr` などの自作の問題が判定サイトの問題の間に散らばっていました。判定サイトの問題と同じく id の先頭で出どころが分かるようにし、自作の問題が一覧でひとまとまりに並ぶようにしました。`self/` の段だけを id に入れない案もありましたが、「id は置き場所の各段を繋いだもの」という決まりに例外ができるので採りませんでした。

族のディレクトリ (`gf2-64/`、`modulo-test/`、`constexpr/` など) は中身の形を変えずに移し、族を持たない 6 問も `self/` の直下に入れました。`problems/_shared/` は動かしていません。提出は `#include "_shared/gf2-64/_common.hpp"` の形で引いているので、動かすと提出のソースが変わって測り直しになります。問題の中の相対の include (`../common.hpp` など) は、問題のディレクトリごと動くのでそのまま通ります。

### 記録は jsonl の名前と problem の欄を書き換えて移しました

キーの材料に問題の id は入っておらず、問題の `-I` も印に置き換わっているので (「キーの材料では問題のディレクトリの -I を印にします」)、置き場所と id を変えてもキーは変わりません。記録は問題の id ごとの jsonl に入っているので、results の `problems/<id>.jsonl` を `problems/self-<id>.jsonl` に名前を変え、各行の `problem` を新しい id に書き換えました。main の push で立つ run の plan が新しい id の記録を読めるように、results を先に push しています。自作の問題のテストデータは保管庫に置かず毎回作り直すので (`source = "local"` か `"none"`)、保管庫のアセット名は動かす必要がありませんでした。

移した時点では、全モデルモードの run 36126484334 (schedule、10:54 UTC に 7038992c で立ったもの) がまだ走っていました。push の前に見たのは直近の数件の run だけで、これを見落としていました。この run は 4 時間後の collect で、51 問ぶん 3,162 行を古い id のまま results に書き戻しました (ba914df9)。どれも今のソースで現行の記録で、新しい id のファイルに無い測定だったので、各行の `problem` を新しい id に書き換えて新しい id のファイルの末尾へ足し、古いファイルを消しました (95497f6)。全モデルモードの未計測 (提出とモデルの組) は 3,666 件から 1,076 件になりました。README の手順には、走っている run を `gh run list --status in_progress` で確かめることを足しています。

記録の `cxxflags` の欄は書き換えていません。この欄は測ったときに実際に使ったフラグで、動かす前の記録には前の場所の `-Iproblems/gf2-64/pow` が残っています。

### ソースへのリンクは測ったときの場所を使います

提出のソースへのリンクは記録の `judge_sha` のコミットを指すので、今の場所と組み合わせると、動かす前に測った記録のリンクが 404 になります。2026-09-23 の階層化のときは同じ日にキーの材料が変わって全部を測り直したので、404 になるのは参考の記録だけでした。今回は測り直しが起きないので、51 問の現行の記録がすべて 404 になります。そこで、記録の `cxxflags` から問題の `-I` を拾い、測ったときの問題のディレクトリとしてリンクに使うようにしました (`site.build.measured_dir`)。問題ページの JSON の行には、その場所が今の `dir` と違うときだけ `dir` が入ります。2026-09-23 より前に測った参考の記録のリンクも、これで当時の平らな場所を指すようになりました。

`pj problems import` は、元の問題の URL を持たない自己検証のテストに `self-` を付けた id を与え、`dir_for_new` はその問題を `problems/self/` の下に書き出します。族のディレクトリは人が決めるので、`self/` の中では平らに置きます。

## 静的メンバ関数の入口を自由関数にした記録

2026-09-26 に、提出の入口を構造体の静的メンバ関数 (`GF2_64Op::run`、`Solver::run`、`Conv::run` など) から自由関数 `run` に変えました。対象は、ハーネスが静的メンバ関数を呼んでいた 45 問 (self が 14、yosupo が 31) と、その提出 330 本です。どれも旧 judge の `algos/*.hpp` の書き方を引き継いだもので、構造体は関数 1 本を包む名前空間の代わりにしかなっていませんでした。提出はヘッダとして base.cpp に include され、1 つの翻訳単位で組まれるので、自由関数にしてもリンクで困ることはありません。状態を持つ `Solver` (インスタンスを作る問題) と、Library のクラスを `using MP = ...` で差し替える modulo-test はそのままです。

入口の名前は `solve` ではなく `run` にしました。gf2-64 の log の提出など 35 本が `solve` を補助の関数の名前に使っていて、入口と重なるからです。gcd-test の `G::gcd` も `run` にしました。`gcd` のままだと、`using namespace std` 越しに見える `std::gcd` と重なります。

### static は関数なら inline に替えました

書き換えは機械的に行いました。`struct 型 {` と閉じの `};`、アクセス指定を外し、1 段字下げを戻しています。メンバの `static` は、関数なら `inline` に替え、データなら外しました (`static constexpr` は `constexpr` になります)。クラスの中で定義した関数は暗黙に inline で、GCC は inline と宣言された関数とそうでない関数で、インライン展開のしきい値を分けています (`max-inline-insns-single` と `max-inline-insns-auto`)。`inline` を付けずに外へ出すと補助の関数の展開のされ方が変わり、書き方と関係のない差が計測に乗るおそれがあります。

書き換えの前後でトークン列 (空白とコメントを除いたもの) を比べ、消えたのが外枠とアクセス指定と `static` だけで、足したのが `inline` だけであることを全ファイルで確かめました。

### 名前の見え方が変わって 2 本が組めなくなりました

クラスの中では、メンバの名前が外側の同じ名前を隠し、メンバ関数どうしは定義の順に関係なく呼べます。外へ出すと、この 2 つが効かなくなります。全提出を x64-gcc と arm-gcc のフラグ、手元の clang で `-fsyntax-only` を付けて組んだところ、次の 2 本が新しく組めなくなっていました。

stern-brocot-tree の `fastest.hpp` は、入口の `run` から、あとで定義する `solve_one` を呼んでいました。`run` を後ろへ移しています。factorize の `mixed_fj64_bgcd.hpp` は primality-test の提出を丸ごと include していて、その提出の入口も `run` になったので、同じ引数の `run` が 2 つになりました。include するあいだだけ `#define run primality_test_run` で別名にしています。

外へ出した関数と同じ名前の関数が標準ライブラリやファイルの中にあって、多重定義で黙って別の関数が呼ばれる形も探しましたが、見つかりませんでした。組めなかったものはほかに 2 本あります (convolution-mod-2-64 の `yosupo_190648_ntt_crt.hpp` と、arm-gcc での nim-product-64 の `yosupo_337840.hpp`)。どちらも書き換える前から CE でした。

ハーネスと全提出のソースが変わるので、45 問の記録 (results e10202fc の時点で 27,809 行) は全部参考に落ち、測り直しになりました。0c868d6 の run 36221072644 で、今ある提出の 1,320 組 (提出と環境の組) を測り直しました。判定が変わったのは 2 組で、matrix-det-arbitrary-mod の `yosupo_fastest.hpp` が AMD EPYC 9V74 で AC から RE (SIGILL) になりました。この提出は x86-64 では AVX-512 を前提に組んでいます。測ったジョブのマシンは、同じ 9V74 なのに `/proc/cpuinfo` に AVX-512 の命令が 1 つも無く、ほかのジョブのマシンには 13 個ありました。VM が CPU の機能を隠していたためで、書き換えとは関係ありません。

## raw の提出のメモリに pj のピーク RSS が乗っていた記録

2026-09-26 に、`self-gf2-64-io` が 4 環境すべてで MLE (528 から 546 MB) になっているのを本人が見つけました。プログラム自身は 2 MB ほどしか使いません (手元の Mac で 1.8 MB)。raw の提出はハーネスを持たないので、メモリは `wait4` の `ru_maxrss` に落ちていて、Linux ではそこに `pj` のピーク RSS が乗っていました。CI の run のジョブの `pj` は記録を全部メモリに持つので、results が増えるにつれて 520 から 546 MB まで育っていました。同じ行の gcc と clang のメモリが KB の桁まで一致していたのが目印です。
