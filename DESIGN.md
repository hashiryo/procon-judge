...
取得に失敗した問題は警告を出して飛ばします。1 問取れなかっただけで、走れる問題の記録まで落とさないためです。判定サイトが落ちている回でも他の問題の記録は残り、取りこぼしは次の実行が拾います。飛ばしたぶんは要約の「テストデータ未取得」に出ます。
...
提出の列だけ flex にしました。セル全体に ellipsis を掛けると、名前が長いときに後ろの印から先に消えます。名前だけを詰めて、印は押し出されないようにしてあります。表の最小幅を 1120 から 1180 に広げたのは、今ある名前に印を足しても収まるようにするためです。
