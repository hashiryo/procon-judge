...
まだ作っていないものがあります。1 提出の記録と時系列を出す `submissions/<問題 id>/<提出名>.html` と `data/submissions/...`、それにヘッダから提出を引く `data/headers/<path>.json` です。ライブラリ側のサイトを作るときに足します。
...
`lib` の列は記録の `library_sha` で、Library のそのコミットへ飛びます。提出が `mylib/...` を include していれば、中身は向こうにあるからです。
...
ライブラリの URL はこのリポジトリが持ちません。設計が「このリポジトリはライブラリという概念を持ちません」と言っているとおりです。`pj site build --library-url` で呼ぶ側から渡す形にして、値は judge.yml に置きました。渡さなければリンクは出ません。
