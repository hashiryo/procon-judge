...
| `schedule` | 取りこぼしと、新しい CPU モデルの発掘。1 日 1 回 |
...
2. 既知のモデルすべてで 0 件なら、その環境のジョブを立てません。未知のモデルに当たれば仕事はありますが、それは計画された仕事ではないので `schedule` の回に任せます。push は計画された仕事、定期実行は発掘、という分担です。
