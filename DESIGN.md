...
代わりに `run` が同じ並びを組み直します。並びは記録と問題定義と `lib/` だけから決まるので、同じものを読めば同じ順になります。`pj.plan.for_env` を `run` からも呼んでいて、順番を決める規則が 2 か所に増えないようにしてあります。ジョブ名は `name:` で `run (x64-gcc #0)` の形に上書きしました。
